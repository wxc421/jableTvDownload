...
func NewTask(output string, url string) (*Downloader, error) {
	result, err := parse.ParseM3u8FromUrl(url)
	if err != nil {
		return nil, err
	}
	tsFolder := filepath.Join(output, tsFolderName)
...
func (d *Downloader) download(segIndex int) error {
	...
	proxyClient, err := client.GetProxyClient()
	resp, err := proxyClient.R().Get(tsUrl)

	if err != nil {
		return fmt.Errorf("request %s, %v", tsUrl, err)
	}
	...
	bytes, err := io.ReadAll(bytes2.NewReader(resp.Body()))
	if err != nil {
...
func (d *Downloader) merge() error {
	...
	output := filepath.Base(d.folder) + ".ts"
	mFilePath := filepath.Join(filepath.Dir(d.folder), output)
	mFile, err := os.Create(mFilePath)
	if err != nil {
		return fmt.Errorf("create %v file failed：%s", output, err.Error())
	}
